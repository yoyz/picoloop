...
#pragma once
#ifndef TWITCHHELM_MIDI_LOOKUP_H
#define TWITCHHELM_MIDI_LOOKUP_H
...
namespace twytchhelmmopo {
  ...
  class MidiLookupSingleton {
    public:
      MidiLookupSingleton() {
        for (int i = 0; i < HELM_MAX_CENTS + 2; ++i) {
          frequency_lookup_[i] = utils::midiCentsToFrequency(i);
        }
      }

      mopo_float centsLookup(mopo_float cents_from_0) const {
        mopo_float clamped_cents = utils::clamp(cents_from_0, 0.0f, HELM_MAX_CENTS);
    ...
    private:
      mopo_float frequency_lookup_[HELM_MAX_CENTS + 2];
  };
