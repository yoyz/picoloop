...
namespace twytchhelmmopo {
  ...
  Voice* VoiceHandler::grabVoice() {
    ...
    HELM_MOPO_ASSERT(active_voices_.size());
  ...
  void VoiceHandler::noteOn(mopo_float note, mopo_float velocity, int sample, int channel) {
    HELM_MOPO_ASSERT(sample >= 0 && sample < buffer_size_);
    HELM_MOPO_ASSERT(channel >= 0 && channel < NUM_MIDI_CHANNELS);
