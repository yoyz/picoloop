...
#pragma once
#ifndef TWITCHHELM_PROCESSOR_H
#define TWITCHHELM_PROCESSOR_H
...
namespace twytchhelmmopo {
  ...
  namespace cr {
    struct Output : public ::twytchhelmmopo::Output {
      Output() : ::twytchhelmmopo::Output(1) { }
    };
  } // namespace cr
  ...
  class Processor {
    public:
      ...
      inline Input* input(unsigned int index = 0) const {
        HELM_MOPO_ASSERT(index < inputs_->size());
      ...
      inline Output* output(unsigned int index = 0) const {
        HELM_MOPO_ASSERT(index < outputs_->size());
