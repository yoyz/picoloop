...
namespace twytchhelmmopo {
  ...
  void StateVariableFilter::process() {
    HELM_MOPO_ASSERT(inputMatchesBufferSize(kAudio));
    ...
    mopo_float cutoff = utils::clamp(input(kCutoff)->at(0), float(MIN_CUTTOFF), sample_rate_);
    mopo_float resonance = utils::clamp(input(kResonance)->at(0),
                                        float(MIN_RESONANCE), float(MAX_RESONANCE));
  ...
  void StateVariableFilter::computePassCoefficients(mopo_float blend,
                                                    ...
                                                    bool db24) {
    HELM_MOPO_ASSERT(resonance > 0.0f);
    HELM_MOPO_ASSERT(cutoff > 0.0f);

    if (db24)
      resonance = sqrt(resonance);

    mopo_float g = tan(PI * utils::min(cutoff / sample_rate_, 0.5f));
    mopo_float k = 1.0f / resonance;

    mopo_float low_pass_amount = sqrt(utils::clamp(1.0f - blend, 0.0f, 1.0f));
    mopo_float band_pass_amount = sqrt(utils::clamp(1.0f - fabsf(blend - 1.0f), 0.0f, 1.0f));
    mopo_float high_pass_amount = sqrt(utils::clamp(blend - 1.0f, 0.0f, 1.0f));
  ...
  void StateVariableFilter::computeShelfCoefficients(Shelves choice,
                                                     mopo_float cutoff,
                                                     mopo_float gain) {
    HELM_MOPO_ASSERT(gain >= 0.0);
    HELM_MOPO_ASSERT(cutoff > 0.0);
