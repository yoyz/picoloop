...
#pragma once
#ifndef TWYTCH_WAVE_H
#define TWYTCH_WAVE_H
    ...
    public:
      WaveLookup() {
        ...
        for (int i = 0; i < LOOKUP_SIZE + 1; ++i)
          sin_[i] = sin((2 * TWYTCH_PI * i) / LOOKUP_SIZE);

        // Square lookup table.
        for (int i = 0; i < LOOKUP_SIZE + 1; ++i) {
          int p = i;
          mopo_float scale = 4.0 / TWYTCH_PI;
        ...
        for (int i = 0; i < LOOKUP_SIZE + 1; ++i) {
          ...
          int p = i;
          mopo_float scale = 2.0 / TWYTCH_PI;
        ...
        for (int i = 0; i < LOOKUP_SIZE + 1; ++i) {
          int p = i;
          mopo_float scale = 8.0 / (TWYTCH_PI * TWYTCH_PI);
      ...
      inline mopo_float fullsin(mopo_float t) const {
        mopo_float integral;
        mopo_float fractional = twytchutils::mod(t * LOOKUP_SIZE, &integral);
        int index = integral;
      ...
      inline mopo_float pyramid(mopo_float t, int harmonics) const {
        ...
        for (size_t i = 0; i < squares; ++i) {
          out += square(twytchutils::mod(phase, &integral), harmonics);
  ...
  class Wave {
    public:
      ...
      static inline mopo_float triangle(mopo_float t) {
        mopo_float integral;
        return fabs(2.0 - 4.0 * twytchutils::mod(t + 0.75, &integral)) - 1;
      }
      ...
      static inline mopo_float hannwave(mopo_float t) {
        return 0.5 * (1.0 - cosf(2.0 * TWYTCH_PI * t));
      }
      ...
      static inline mopo_float pyramid(mopo_float t) {
        ...
        mopo_float integral;
        for (size_t i = 0; i < squares; ++i) {
          out += square(twytchutils::mod(phase, &integral));
          phase += phase_increment;
        }
        out /= squares;
        return out;
