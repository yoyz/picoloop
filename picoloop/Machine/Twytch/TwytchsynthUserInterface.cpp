...
void TwytchsynthUserInterface::handle_key_osc()
{
  ...
  helper_handle_key_one_button(GLOBALMENU_OSC, MENU_PAGE0_SUB1, KEY_REPEAT_INTERVAL_LONG,
			       OSC3_TYPE,
			       1);
  ...
  helper_handle_key_two_button(GLOBALMENU_OSC, MENU_PAGE0_SUB3, KEY_REPEAT_INTERVAL_SMALLEST,
			       OSC1_UNISON,
			       OSC2_UNISON,
			       1,1);

  helper_handle_key_two_button(GLOBALMENU_OSC, MENU_PAGE0_SUB4, KEY_REPEAT_INTERVAL_SMALLEST,
			       OSC1_UNISONDT,
			       OSC2_UNISONDT,
			       1,1);

  helper_handle_key_two_button(GLOBALMENU_OSC, MENU_PAGE0_SUB5, KEY_REPEAT_INTERVAL_LONG,
			       OSC1_SCALE,
			       OSC2_SCALE,
			       1,1);
  
  helper_change_sub_menu(MENU_PAGE0_SUB5); 
}
...
void TwytchsynthUserInterface::handle_key_vco()
{
  ...
  helper_handle_key_two_button(GLOBALMENU_VCO, MENU_PAGE0_SUB2, KEY_REPEAT_INTERVAL_SMALLEST,
			       OSC1_DETUNE,
			       OSC1_MOD,
			       1,1);

  helper_handle_key_two_button(GLOBALMENU_VCO, MENU_PAGE0_SUB3, KEY_REPEAT_INTERVAL_SMALLEST,
			       OSC3_AMP,
			       OSC4_AMP,
			       1,1);

  helper_change_sub_menu(MENU_PAGE0_SUB3); 
}
...
void TwytchsynthUserInterface::handle_key_lfo()
{
  ...
  helper_handle_key_one_button(GLOBALMENU_LFO, MENU_PAGE0_SUB2, KEY_REPEAT_INTERVAL_SMALLEST,
			       LFO_TYPE,
			       1);

  // change GLOBALMENU_LFO SUBMENU  
...
void TwytchsynthUserInterface::handle_key_fltr()
{
  ...
  helper_handle_key_two_button(GLOBALMENU_FLTR, MENU_PAGE0_SUB2, KEY_REPEAT_INTERVAL_SMALLEST,
			       FILTER1_SATURATION,
			       FILTER1_FEEDBACK,
			       1,1);
  ...
  if (menu_cursor==GLOBALMENU_FLTR)
    {
      ...
      if (menu_sub==MENU_PAGE0_SUB2)     sprintf(str_line2,"FLTR DIST/FDBK"); 
    }
  if (menu_cursor==GLOBALMENU_VCO)
    {
      if (menu_sub==MENU_PAGE0_SUB0)      sprintf(str_line2,"Velocity/VCOMIX");
      if (menu_sub==MENU_PAGE0_SUB1)      sprintf(str_line2,"envdepth/keytrack");
      if (menu_sub==MENU_PAGE0_SUB2)      sprintf(str_line2,"Detune/CrossMod");
      if (menu_sub==MENU_PAGE0_SUB3)      sprintf(str_line2,"Sub/Noise");
    }
    if (menu_cursor==GLOBALMENU_LFO)
    {
      ...
      if (menu_sub==MENU_PAGE0_SUB2)      sprintf(str_line2,"LFO  Type");
    }
    if (menu_cursor==GLOBALMENU_OSC)
      {
	if (menu_sub==MENU_PAGE0_SUB0)    sprintf(str_line2,"OSC WAVEFORM");
	if (menu_sub==MENU_PAGE0_SUB1)    sprintf(str_line2,"SUB WAVEFORM");
	if (menu_sub==MENU_PAGE0_SUB2)    sprintf(str_line2,"LFO WAVEFORM");
	if (menu_sub==MENU_PAGE0_SUB3)    sprintf(str_line2,"UNI OSC");
	if (menu_sub==MENU_PAGE0_SUB4)    sprintf(str_line2,"UNI DETUNE");
      }
...
void TwytchsynthUserInterface::display_board_vco()
{
  if (menu_cursor == GLOBALMENU_VCO)
    {
      if (menu_sub>MENU_PAGE0_SUB3)
	{ menu_sub=MENU_PAGE0_SUB0; }

      if (menu_sub    == MENU_PAGE0_SUB0) display_board_two_param(VCO_MIX,VELOCITY);
      if (menu_sub    == MENU_PAGE0_SUB1) display_board_two_param(KEYTRACK,ENV1_DEPTH);
      if (menu_sub    == MENU_PAGE0_SUB2) display_board_two_param(OSC1_DETUNE,OSC1_MOD);
      if (menu_sub    == MENU_PAGE0_SUB3) display_board_two_param(OSC3_AMP,OSC4_AMP);
    }  
}
...
void TwytchsynthUserInterface::display_board_osc()
{
  if (menu_cursor==GLOBALMENU_OSC)
    {
      if (menu_sub>MENU_PAGE0_SUB5)
	{ menu_sub=MENU_PAGE0_SUB0; }
      
      if (menu_sub   ==MENU_PAGE0_SUB0) display_board_two_param_text(OSC1_TYPE,OSC2_TYPE);
      if (menu_sub   ==MENU_PAGE0_SUB1) display_board_one_param_text(OSC3_TYPE);
      if (menu_sub   ==MENU_PAGE0_SUB2) display_board_two_param_text(LFO1_WAVEFORM,LFO2_WAVEFORM);
      if (menu_sub   ==MENU_PAGE0_SUB3) display_board_two_param(OSC1_UNISON,OSC2_UNISON);
      if (menu_sub   ==MENU_PAGE0_SUB4) display_board_two_param(OSC1_UNISONDT,OSC2_UNISONDT);
      if (menu_sub   ==MENU_PAGE0_SUB5) display_board_two_param_number(OSC1_SCALE,OSC2_SCALE);
    }
}
...
void TwytchsynthUserInterface::display_board_lfo()
{
  ...
  if (menu_cursor==GLOBALMENU_LFO)
	...
	{ menu_sub=MENU_PAGE0_SUB0; }
      ...
      // LFOPITCH   // PITCHBEND
      if (menu_sub==MENU_PAGE0_SUB0) display_board_two_param(LFO1_ENV_AMOUNT,LFO1_FREQ);
      if (menu_sub==MENU_PAGE0_SUB1) display_board_two_param(LFO2_ENV_AMOUNT,LFO2_FREQ);
...
void TwytchsynthUserInterface::display_board_fltr()
{
  if (menu_cursor==GLOBALMENU_FLTR)
	...
	{ menu_sub=MENU_PAGE0_SUB0; }
      ...
      if (menu_sub==MENU_PAGE0_SUB2) display_board_two_param(FILTER1_SATURATION,FILTER1_FEEDBACK);
    }
}
