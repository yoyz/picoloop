...
#pragma once
#ifndef TWITCHHELM_HELM_COMMON_H
#define TWITCHHELM_HELM_COMMON_H
...
namespace twytchhelmmopo {
  ...
  typedef std::map<std::string, twytchhelmmopo::Value *> control_map;
  ...
  const twytchhelmmopo::cr::Value synced_freq_ratios[] = {
    cr::Value(1.0 / 128.0),
    cr::Value(1.0 / 64.0),
    cr::Value(1.0 / 32.0),
    cr::Value(1.0 / 16.0),
  ...
  class ValueDetailsLookup {
    public:
      ...
      const ValueDetails& getDetails(const std::string& name) const {
        ...
        HELM_MOPO_ASSERT(details != details_lookup_.end());
