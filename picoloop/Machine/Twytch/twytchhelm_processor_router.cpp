...
namespace twytchhelmmopo {
  ...
  void ProcessorRouter::process() {
    ...
    HELM_MOPO_ASSERT(num_processors != 0);
  }
  ...
  void ProcessorRouter::addProcessor(Processor* processor) {
    HELM_MOPO_ASSERT(processor->router() == 0 || processor->router() == this);
    (*global_changes_)++;
    local_changes_++;
  ...
  void ProcessorRouter::removeProcessor(const Processor* processor) {
    ...
    HELM_MOPO_ASSERT(local_pos != local_order_.end());
  ...
  void ProcessorRouter::reorder(Processor* processor) {
    ...
    HELM_MOPO_ASSERT(new_order.size() == processors_.size());
  ...
  void ProcessorRouter::removeFeedback(Feedback* feedback) {
    ...
    HELM_MOPO_ASSERT(pos != global_feedback_order_->end());
    ...
    HELM_MOPO_ASSERT(local_pos != local_feedback_order_.end());
