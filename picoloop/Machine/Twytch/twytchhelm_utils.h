...
#pragma once
#ifndef TWITCHHELM_UTILS_H
#define TWITCHHELM_UTILS_H
...
namespace twytchhelmmopo {
  ...
  namespace {
    ...
    const mopo_float HELM_MIDI_0_FREQUENCY = 8.1757989156;
    const int HELM_NOTES_PER_OCTAVE = 12;
    const int HELM_CENTS_PER_NOTE = 100;
    const int CENTS_PER_OCTAVE = HELM_NOTES_PER_OCTAVE * HELM_CENTS_PER_NOTE;
    const int HELM_MAX_CENTS = MIDI_SIZE * HELM_CENTS_PER_NOTE;
  ...
  namespace utils {
    ...
    inline mopo_float midiCentsToFrequency(mopo_float cents) {
      return HELM_MIDI_0_FREQUENCY * centsToRatio(cents);
    }

    inline mopo_float midiNoteToFrequency(mopo_float note) {
      return midiCentsToFrequency(note * HELM_CENTS_PER_NOTE);
    }

    inline mopo_float frequencyToMidiNote(mopo_float frequency) {
      return HELM_NOTES_PER_OCTAVE * log(frequency / HELM_MIDI_0_FREQUENCY) / log(2.0);
    }

    inline mopo_float frequencyToMidiCents(mopo_float frequency) {
      return HELM_CENTS_PER_NOTE * frequencyToMidiNote(frequency);
    }
