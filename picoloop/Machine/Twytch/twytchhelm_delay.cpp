...
namespace twytchhelmmopo {
  ...
  void Delay::process() {
    ...
    mopo_float wet = utils::clamp(input(kWet)->at(0), 0.0f, 1.0f);
    ...
    mopo_float new_period = utils::clamp(input(kSampleDelay)->at(0), 2.0f, memory_->getSize() - 1.0f);
  ...
  inline void Delay::tick(int i, const mopo_float* audio, mopo_float* dest) {
    ...
    HELM_MOPO_ASSERT(std::isfinite(dest[i]));
  }
} // namespace twytchhelmmopo
