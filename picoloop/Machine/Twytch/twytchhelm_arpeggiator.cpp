...
namespace twytchhelmmopo {
  ...
  void Arpeggiator::process() {
    ...
    mopo_float gate = utils::interpolate(min_gate, 1.0f, input(kGate)->at(0));
  ...
  std::pair<mopo_float, mopo_float> Arpeggiator::getNextNote() {
    ...
    mopo_float note = base_note + twytchhelmmopo::HELM_NOTES_PER_OCTAVE * current_octave_;
