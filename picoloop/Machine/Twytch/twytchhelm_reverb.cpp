...
#include "twytchhelm_reverb_tuning.h"
...
namespace twytchhelmmopo {
  ...
  Reverb::Reverb() : ProcessorRouter(kNumInputs, 2), current_dry_(0.0), current_wet_(0.0) {
    ...
    for (int i = 0; i < NUM_COMB; ++i) {
      ReverbComb* comb = new ReverbComb(1 + twytchhelmmopo::MAX_SAMPLE_RATE * COMB_TUNINGS[i]);
    ...
    for (int i = 0; i < NUM_COMB; ++i) {
      ...
      ReverbComb* comb = new ReverbComb(1 + twytchhelmmopo::MAX_SAMPLE_RATE * tuning);
    ...
    for (int i = 0; i < NUM_ALL_PASS; ++i) {
      ReverbAllPass* all_pass = new ReverbAllPass(1 + twytchhelmmopo::MAX_SAMPLE_RATE * ALL_PASS_TUNINGS[i]);
    ...
    for (int i = 0; i < NUM_ALL_PASS; ++i) {
      ...
      ReverbAllPass* all_pass = new ReverbAllPass(1 + twytchhelmmopo::MAX_SAMPLE_RATE * tuning);
      Value* time = new cr::Value(tuning);
      addIdleProcessor(time);
      cr::TimeToSamples* samples = new cr::TimeToSamples();
      samples->plug(time);
  ...
  void Reverb::process() {
    ...
    mopo_float wet_in = utils::clamp(input(kWet)->at(0), 0.0f, 1.0f);
