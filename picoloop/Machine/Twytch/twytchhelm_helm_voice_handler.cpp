...
#include "twytchhelm_helm_voice_handler.h"

#include "twytchhelm_fixed_point_oscillator.h"
#include "twytchhelm_gate.h"
#include "twytchhelm_ladder_filter.h"
#include "twytchhelm_noise_oscillator.h"
#include "twytchhelm_resonance_cancel.h"
#include "twytchhelm_helm_lfo.h"
#include "twytchhelm_helm_oscillators.h"
#include "twytchhelm_trigger_random.h"
...
namespace twytchhelmmopo {
  ...
  void HelmVoiceHandler::init() {
    ...
    for (int i = 0; i < twytchhelmmopo::NUM_MIDI_CHANNELS; ++i) {
  ...
  void HelmVoiceHandler::createOscillators(Output* midi, Output* reset) {
    ...
    static const cr::Value sub_transpose(-2 * HELM_NOTES_PER_OCTAVE);
  ...
  void HelmVoiceHandler::setModWheel(mopo_float value, int channel) {
    HELM_MOPO_ASSERT(channel >= 1 && channel <= mopo::NUM_MIDI_CHANNELS);
