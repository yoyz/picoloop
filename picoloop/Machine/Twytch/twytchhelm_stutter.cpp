...
namespace twytchhelmmopo {
  ...
  namespace {
    inline mopo_float computeAmplitude(mopo_float offset, mopo_float period, mopo_float softness) {
      ...
      mopo_float phase = utils::clamp(phase_setup - softness + PI, 0.0f, PI);
  ...
  void Stutter::process() {
    ...
    mopo_float read_softness = utils::max(input(kWindowSoftness)->at(0), float(MIN_SOFTNESS));
    mopo_float end_softness = PI * utils::max(1.0f, 1.0f / read_softness);
    ...
    while (i < buffer_size_) {
      ...
      HELM_MOPO_ASSERT(stutter_period > 0);
      HELM_MOPO_ASSERT(offset_ >= 0);
      HELM_MOPO_ASSERT(samples >= 0 && samples <= buffer_size_);
    }
