...
void CursynthUserInterface::handle_key_osc()
{
  ...
  helper_handle_key_one_button(GLOBALMENU_OSC, MENU_PAGE0_SUB2, KEY_REPEAT_INTERVAL_LONG,
			       OSC2_SCALE,
			       1);
  ...
  if (menu_cursor==GLOBALMENU_OSC)
    {
      ...
      if (menu_sub==MENU_PAGE0_SUB1)      sprintf(str_line2,"LFO WAVEFORM");
    }
...
{
  DPRINTF("CursynthUserInterface::display_board_amp_env()\n");
  if (menu_cursor==GLOBALMENU_AD)
    {
...
void CursynthUserInterface::display_board_osc()
{
  if (menu_cursor==GLOBALMENU_OSC)
	...
	{ menu_sub=MENU_PAGE0_SUB0; }
      ...
      if (menu_sub   ==MENU_PAGE0_SUB1) display_board_two_param_text(LFO1_WAVEFORM,LFO2_WAVEFORM);
      if (menu_sub   ==MENU_PAGE0_SUB2) display_board_one_param_number(OSC2_SCALE);	
    }
}
