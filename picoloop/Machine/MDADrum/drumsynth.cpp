...
int ds2mem(char *dsfile, short **buffer, float t, float o, float n, float b, 
                                                 float tune, float time)
{
  ...
  *buffer=malloc(sizeof(short)*wavewords);
  memcpy(*buffer,wave,wavewords*sizeof(short));
  printf("MDA ds2mem wave:0x%08.8X buffer:0x%08.8X wavewords:%d\n",wave,*buffer,wavewords);
  return wavewords;
}
...
int ds2wav(char *dsfile, char *wavfile)
{
  ...
  busy = 0; //success!! 
  printf("MDA ds2wav wave:0x%08.8X\n",wave);
  return 0;
}
