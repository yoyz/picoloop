...
void MDADrumUserInterface::display_board_text()
{
  ...
  if (menu_cursor==GLOBALMENU_FLTR)
    ...
    {
      if (menu_sub==MENU_PAGE0_SUB0)      sprintf(str_line2,"KIT/INST");
    }
    if (menu_cursor==GLOBALMENU_VCO)
    {
      if (menu_sub==MENU_PAGE0_SUB0)      sprintf(str_line2,"NO VCO");
    }
...
{
  DPRINTF("MDADrumUserInterface::display_board_amp_env()\n");
  if (menu_cursor==GLOBALMENU_AD)
    {
	...
	{ menu_sub=MENU_PAGE0_SUB0; }
      ...
      if (menu_sub==MENU_PAGE0_SUB1) display_board_two_param(AMP,TRIG_TIME_DURATION);	
    }  
}
