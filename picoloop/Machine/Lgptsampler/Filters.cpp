...
void set_filter(int channel, filterType_t type, lgptfixed param1,lgptfixed param2,int mix,bool bassyMapping)
{
  ...
  if (param1 != flt->parm1)
  {
    ...
    if (bassyMapping)
    {
      static const lgptfixed fpFreqDivider = fl2fp(1/22050.0f);
      static const lgptfixed fpZeroSix = fl2fp(0.6f);
      static const lgptfixed fpThreeOne = fl2fp(3.1f);

      lgptfixed power = fp_add(fpZeroSix,fp_mul(param1,fpThreeOne));
      lgptfixed frequency = fl2fp(pow(10.0f,fp2fl(power))) ;
...
/*
void filterize(int channel, lgptfixed* buffer, long int size)
{
