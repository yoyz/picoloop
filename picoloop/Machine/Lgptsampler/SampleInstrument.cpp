...
lgptfixed SampleInstrument::feedback_[SONG_CHANNEL_COUNT][FB_BUFFER_LENGTH*2] ;
...
/*			if (loopmode==SILM_OSCFINE) {
	...
	while (offset>127)
  {
	...
	lgptfixed freqFactor=fl2fp(float(pow(2.0,(offset+fineTune)/12.0))) ;
  ...
  // Initialize feedback data
	...
	memset(feedback_[channel],0,FB_BUFFER_LENGTH*2*sizeof(lgptfixed)) ;
...
bool SampleInstrument::Render(int channel,lgptfixed *buffer,int size,bool updateTick) {
  ...
  lastMidiNote_[channel]=rp->midiNote_ ;
	...
	// Process tick-level updates
	  ...
	  // Get additional parameters from variables
		 ...
		 lgptfixed fpcrushvol=fl2fp(crushvol/255.0F) ;
		...
		lgptfixed *result=buffer ;

		// Get volume factor and pan

		lgptfixed volscale=fl2fp(0.003921568627450980392156862745098f) ;
		lgptfixed volfactor=fp_mul(rp->volume_,volscale) ;
	  int pan=fp2i(rp->pan_) ;
	  lgptfixed fixedpanl=panlaw[pan] ;
	  lgptfixed fixedpanr=panlaw[254-pan] ;

		// filter constants

		lgptfixed f_k=fl2fp( 1.0F / 3.0F ) ;
		lgptfixed f_s=FP_ONE - f_k ;
    ...
    short *input=(short *)(wavbuf+2*channelCount*n) ; // input is the current
		...
		lgptfixed fpPos=fl2fp(rp->position_-n) ;  // fpPos is current pos from input
		lgptfixed fpSpeed=rp->speed_ ;     // speed in lgptfixed
		if (rp->reverse_)
    {
			fpSpeed=-rp->speed_ ; 
		} 

    lgptfixed s1,s2,t2,eta,inveta;
		s2=0 ; t2=0 ;
    ...
    lgptfixed zerofive=fl2fp(0.5f) ;

		// Get feedback pointer position & boundary

		lgptfixed *feedbackIn=feedback_[channel]+rp->feedbackIn_*2 ;
		lgptfixed *feedbackStart=feedback_[channel] ;
		lgptfixed *feedbackEnd=feedback_[channel]+FB_BUFFER_LENGTH*2 ;

		// Update feedback data if necessary

		updateFeedback(rp) ;

		lgptfixed *feedbackPick=feedback_[channel]+rp->feedbackOut_*2 ;
		lgptfixed feedbackEta=fp_mul(rp->fbMix_,fl2fp(4.0f));

		lgptfixed f_32767=i2fp(32767/4) ;
		lgptfixed f_m32768=i2fp(-32768/4) ;
		...
		lgptfixed *fltSpeed=flt->speed ;
		lgptfixed *fltHeight=flt->height ;
		lgptfixed fltMix=flt->mix ;
		lgptfixed fltMixInv=FP_ONE-fltMix ;
		lgptfixed *fltDelay=flt->hipdelay ;
		lgptfixed fltParm1=flt->freq ;
		lgptfixed fltParm2=flt->reso ;
		lgptfixed fltDirt=flt->dirt ;

		lgptfixed *fltSpeedPtr=0 ;
		lgptfixed *fltDelayPtr=0 ;
		lgptfixed *fltHeightPtr=0 ;
      ...
      if (*rpFinished) {
			  ...
			  if (rpKrateCount--==0) {
					...
					if (hasUpdaters) {
						...
						pan=fp2i(rp->pan_) ;
						lgptfixed lgptfixedpanl=panlaw[pan] ;
						lgptfixed lgptfixedpanr=panlaw[254-pan] ;
          ...
          s2=fp_mul(s2,volfactor) ;
				...
				// apply filtering if needed
					...
					if (filtering) {
						...
						lgptfixed lpin =fp_mul(s2,fltMixInv) ;
      			lgptfixed hpin = -fp_mul(s2,fltMix) ;
							
    				lgptfixed difr = fp_sub(lpin,*fltHeightPtr);
