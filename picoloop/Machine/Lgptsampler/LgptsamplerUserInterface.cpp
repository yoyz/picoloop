#include "LgptsamplerUserInterface.h"




LgptsamplerUserInterface::LgptsamplerUserInterface()
{
}
...
void LgptsamplerUserInterface::handle_key(int menu)
{
  DPRINTF("LgptsamplerUserInterface::handle_key(%d,%d)\n",menu);
...
void LgptsamplerUserInterface::handle_key_amp_env()
{
  // GLOBALMENU_AD AMP
  // Move Attack Release 
  // Insert/Remove Trig
...
void LgptsamplerUserInterface::handle_key_note()
{
    // GLOBALMENU_NOTE
  // change note
  // copy/paste
...
void LgptsamplerUserInterface::handle_key_osc()
{
...
void LgptsamplerUserInterface::handle_key_vco()
{
  // GLOBALMENU_VCO
  // VCO Menu
  // Change Value
...
void LgptsamplerUserInterface::handle_key_lfo()
{
  // GLOBALMENU_LFO
  // LFO Menu
  // Change Value
...
void LgptsamplerUserInterface::handle_key_fltr()
{
  // GLOBALMENU_FLTR
  ...
  helper_handle_key_two_button(GLOBALMENU_FLTR, MENU_PAGE0_SUB1, KEY_REPEAT_INTERVAL_LONG,
			       FILTER1_ALGO,
			       FILTER1_TYPE,
			       1,1);
...
void LgptsamplerUserInterface::display_board_text()
{
  int  i=0;
  char str_line1[64]="";
  char str_line2[64]="";
  ...
  int  stepdiv=SEQ.getPatternSequencer(cty).getBPMDivider();
    ...
    if (menu_cursor==GLOBALMENU_AD)
    {
      ...
      if (menu_sub==MENU_PAGE0_SUB1)     sprintf(str_line2,"AMP  T/N");	    
    }
  ...
  if (menu_cursor==GLOBALMENU_LFO)
    {
      if (menu_sub==MENU_PAGE0_SUB0)      sprintf(str_line2,"LFOPitch Depth/Speed");
      if (menu_sub==MENU_PAGE0_SUB1)      sprintf(str_line2,"PitchBend Depth/Speed");
    }
...
void LgptsamplerUserInterface::display_board(int menu)
{
  DPRINTF("LgptsamplerUserInterface::display_board(%d,)\n",menu);
  if (menu==GLOBALMENU_AD) 
...
void LgptsamplerUserInterface::display_board_amp_env()
{
  DPRINTF("LgptsamplerUserInterface::display_board_amp_env()\n");
  int  i;
  int  cty=SEQ.getCurrentTrackY();
...
void LgptsamplerUserInterface::display_board_note()
{
  int  i;
  int  j;
  int  cty=SEQ.getCurrentTrackY();
...
void LgptsamplerUserInterface::display_board_vco()
{
  int  i;
  int  cty=SEQ.getCurrentTrackY();
  int  step=SEQ.getPatternSequencer(cty).getStep();
...
void LgptsamplerUserInterface::display_board_osc()
{
  int  i;
  int  cty=SEQ.getCurrentTrackY();
  int  step=SEQ.getPatternSequencer(cty).getStep();
...
void LgptsamplerUserInterface::display_board_lfo()
{
...
void LgptsamplerUserInterface::display_board_fltr()
{
  if (menu_cursor==GLOBALMENU_FLTR)
    {      
