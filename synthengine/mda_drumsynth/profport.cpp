...
float GetPrivateProfileFloat (char * section, char * entry, int def, char * file_name) 
  { 
    return get_private_profile_float(section,entry,def,file_name); 
  } 


  int GetPrivateProfileInt (char * section, char * entry, int def, char * file_name) 
  { 
    return get_private_profile_int(section,entry,def,file_name); 
  } 
  
  int GetPrivateProfileString(char *section, char *entry, char *def,char *buffer, int buffer_len, char *file_name)
  {
    return get_private_profile_string(section,entry,def,buffer,buffer_len,file_name);
  }
