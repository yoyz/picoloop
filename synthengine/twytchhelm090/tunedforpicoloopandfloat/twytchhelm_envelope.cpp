...
#include "twytchhelm_sample_decay_lookup.h"
...
namespace twytchhelmmopo {
  ...
  void Envelope::process() {
    ...
    if (state_ == kAttacking) {
      mopo_float attack = utils::max(input(kAttack)->at(0), 0.000000001f);
    ...
    if (state_ == kDecaying) {
      ...
      current_value_ = utils::max(0.0f, current_value_ - decrement);
      output(kValue)->buffer[0] = current_value_;
    }
  }
} // namespace twytchhelmmopo
