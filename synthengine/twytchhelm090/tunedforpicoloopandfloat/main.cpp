...
int main()
{
  twytchhelmmopo::mopo_float      * out_buffer_f;
  Sint16                * out_buffer_i;
  ...
  int j;
  int fd;
  out_buffer_f=(twytchhelmmopo::mopo_float*)malloc(sizeof(twytchhelmmopo::mopo_float)*8192);
  out_buffer_i=(Sint16*)malloc(sizeof(Sint16*)*8192);
  FILE     * FD;

  twytchhelmmopo::HelmEngine HE;
  //CS.setBufferSize(8192);

  HE.setBufferSize(256);
  HE.setSampleRate(44100);
  HE.noteOn(40,1.0);
  //CS.processAudio((mopo::twytchhelmmopo_float*)out_buffer, n_frames);
  FD=fopen("audioout","w+");
